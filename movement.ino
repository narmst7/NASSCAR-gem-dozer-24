...
const int cArmServoUp = 1900;       // Value for shoulder of arm fully up
...
const double distancePerRev = 13.2;  //circumference of the wheels in cm
//
//=====================================================================================================================
          ...
          if (motorsEnabled) {          // run motors only if enabled
            if (timeUp2sec) {           // update drive state after 2 seconds
              switch (driveIndex) {     // cycle through drive states
                ...
                case 9:
                    robotModeIndex = 0;  // reset drive index
                    servoReset();
                    calibrated = false;
                    driveIndex = 0;        // reset drive index
                    timeUp2sec = false;    // reset 2 second timer
...
// reset claw and arm to default and up positions
void servoReset() {
  Bot.ToPosition("S1", cClawServoOpen);  // update claw servo position to fully open
  Bot.ToPosition("S2", cArmServoUp);     // update shoulder servo position to fully up
  servoReseted = true;                   //update flag
}
